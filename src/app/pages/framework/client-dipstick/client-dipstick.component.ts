...
@Component({
  selector: 'app-client-dipstick',
  templateUrl: './client-dipstick.component.html',
  styleUrls: ['./client-dipstick.component.scss'],
})
export class ClientDipstickComponent {
  constructor(public commonService: PieworksCommonService) {
    ...
    this.startDate = '2020-01-01'.split(' ')[0];

    if (commonService.clientIds.length > 0) {
      this.getCandidateData();
    }
    else
    {
      setTimeout(()=>{this.getCandidateData()},1000)
    }
  }
  ...
  getCandidateData() {
    // change should be done when we have multiple client ids
    var url =
      ...
      this.endDate +
      '&clientId=' +
      this.commonService.clientIds[0];
